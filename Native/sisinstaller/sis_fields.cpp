...
namespace eka2l1 {
    namespace loader {
        static constexpr std::uint64_t MAX_ALLOWED_COMPRESSED_DATA = 209715200; // 200 MB
        ...
        sis_parser::sis_parser(const std::string &path) {
            stream = std::make_shared<common::ro_std_file_stream>(path, true);
        }
...
#define PARSE_ELEMENT_TYPE(element_case, handler_result_type, handler)                                               \
    case sis_field_type::element_case:                                                                               \
        ...
        sis_compressed sis_parser::parse_compressed(bool no_extract) {
            ...
            if (compressed.uncompressed_size >= MAX_ALLOWED_COMPRESSED_DATA) {
                throw sis_uncompressed_data_too_large_exception();
            }
